...
def list_diagnoses():
    ...
    try:
        # JOIN implícito (PostgREST) gracias a la FK:
        # diagnostico_foto.diagnostico (text) -> diagnostico.enfermedad (PK text)
        q = (
            supabase
            .table("diagnostico_foto")
            .select("iddiagnostico, imagen_url, fecha, idUsuario, diagnostico, diagnostico(descripcion, causas, prevencion, tratamiento)")
